...
const EditCommuteModal = ({ show, commute, onClose, refresh }) => {
  ...
  useEffect(() => {
    if (show && commute) {
      setForm({
        codeofemp: commute.codeofemp,
        dateofstart: commute.dateofstart ? commute.dateofstart.split("T")[0] : "",
        dateofend: commute.dateofend ? commute.dateofend.split("T")[0] : "",
        distofstart: commute.distofstart,
        distofend: commute.distofend,
        venderserviceid: commute.venderserviceid
      });
  ...
  const handleSubmit = async (e) => {
    ...
    const updated = { ...commute, ...form }; // include original UUIDs
    await EmployeeCommuteController.updateCommute(updated);
    refresh();
    onClose();
  };
