...
const DataTable = ({ columns, rows, onEdit, onDelete, onSendEmail, onReset, showActions,personId }) => {
                        ...
                        {rows.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => (
                            <TableRow key={row.id}>
                                {columns.map((col) => (
                                    ...
                                    {col.field === 'active' ? (
                                      // Conditionally render the active field
                                      <Box
                                        sx={{
                                          ...
                                          py: 0.5,
                                          borderRadius: 2,
                                          color: row[col.field] ? 'success.main' : 'error.main',
                                          bgcolor: row[col.field] ? 'success.light' : 'error.light',
                                          fontWeight: 'bold',
                                          textAlign: 'center',
                                        }}
                                      >
  ...
  <TableCell>
    ...
    >
      ...
      {/* Hide Reset button when personId matches row.id and for "供應商窗口" */}
{personId !== row.id && row.rolename !== "供應商窗口" && row.rolename !== "碳記帳士"  && (
  <Button
    variant="contained"
    color={row.active ? "error" : "success"} // Red for Deactivate, Green for Activate
    size="small"
    onClick={() => onReset(row.id)}
  >
    {row.active ? 'Deactivate' : 'Activate'} {/* Change button label based on row.active */}
  </Button>
      )}
      {personId !== row.id && (
      <Button
