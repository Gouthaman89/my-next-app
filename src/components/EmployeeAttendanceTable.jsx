...
const EmployeeAttendanceTable = ({ data = [], refresh }) => {
  ...
  const columns = [
    ...
    { key: 'dateofcomm', label: '日期' },
    { key: 'transportation', label: '交通方式' },
    { key: 'startaddress', label: '出發地' },
    { key: 'destinationaddress', label: '目的地' },
    { key: 'dateofstart', label: '開始日' },
    { key: 'dateofend', label: '結束日' },
    { key: 'status', label: '狀態' },
    { key: 'co2e', label: 'co2e' },
  ];
  ...
  const handleConfirmDelete = async () => {
    ...
    await EmployeeAttendanceController.deleteRecord(deletingRow);
    setDeletingRow(null);
    refresh();
  };

  /* ---------- helpers ---------- */
  const getErrorCol = (status) =>
    status?.startsWith('X |') ? status.split('|')[1]?.trim() : null;
              ...
              return (
                <tr key={row.uuid}>
                  {columns.map((col) => {
                    ...
                    return (
                      <td key={col.key} style={{ padding: 8, backgroundColor: bg }}>
                        {col.key === 'status' ? (
                          <span
                            style={{
                              color: value?.startsWith('X')
                                ? 'red'
                                : value?.startsWith('V')
                                ? 'green'
                                : undefined,
